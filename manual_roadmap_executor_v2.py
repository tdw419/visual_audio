#!/usr/bin/env python3
"""
Manual Roadmap Task Executor - executes pending tasks, skipping manual tests
"""
import re
import sys
import os
...
def parse_roadmap():
    ...
    # Skip tasks with manual tests
    automated_tasks = [
        t for t in filtered_tasks
        if t['test_command'] and 'manual' not in t['test_command'].lower()
    ]
...
def main():
    ...
    if not pending:
        print("No pending tasks with automated tests found")
        return 0
    
    priority_order = {'CRITICAL': 0, 'HIGH': 1, 'MEDIUM': 2, 'LOW': 3}
    pending.sort(key=lambda t: priority_order.get(t['priority'], 99))
    
    print(f"Found {len(pending)} pending tasks with automated tests:")
