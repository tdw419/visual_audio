#!/usr/bin/env python3
"""
Run with much larger instruction limit per dispatch.
"""
...
# Use 100K instructions per dispatch
max_instr_arr = np.array([100000], dtype=np.uint32)
...
# Run 10 dispatches (up to 1M instructions)
print("Running up to 1M instructions (10 dispatches x 100K each)...")
for iteration in range(10):
    ...
    if mtimecmp != last_mtimecmp:
        print(f"\n  [{instr_count:6d} instrs] Timer armed: mtimecmp = 0x{mtimecmp:016x}")
        last_mtimecmp = mtimecmp
    
    # Check for interrupt
    if total_irq_count != last_irq:
        print(f"  [{instr_count:6d} instrs] Interrupt! total_irq={total_irq_count}")
        last_irq = total_irq_count
    
    print(f"  Iter {iteration}: {instr_count:6d} instrs, PC=0x{pc:08x}, timer_irq={timer_irq_count}, total_irq={total_irq_count}, mtimecmp=0x{mtimecmp:016x}")
