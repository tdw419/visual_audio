#!/usr/bin/env python3
"""
Level 5c GPU test - minimal sync version, read UART only at end.
"""
...
def main():
    ...
    # Execute - batch test uses 50k steps, use appropriate batch size
    total_steps = 50_000

    # Set CPU state with full steps count
    state_data = np.array(
    ...
    # Execute in one go (step() handles batching internally)
    print(f"Executing {total_steps} steps...")
    core.step(steps=total_steps)

    # Read UART output once at end
