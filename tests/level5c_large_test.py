#!/usr/bin/env python3
"""
Test larger step counts to find where performance collapses.
"""
...
def main():
    ...
    # Test larger step counts
    print("Testing larger step counts:")
    print("Steps\tTime(s)\tSteps/sec")
    print("-" * 50)

    test_sizes = [5000, 10000, 20000, 30000, 40000, 50000]

    for steps in test_sizes:
        state_data = np.array(
        ...
        rate = steps / elapsed if elapsed > 0.001 else float('inf')
        print(f"{steps}\t{elapsed:.3f}\t\t{rate:.0f}")

        # Check if we're approaching timeout
        if elapsed > 50:
            print(f"Warning: {steps} steps took {elapsed:.1f}s - approaching timeout")
            break
