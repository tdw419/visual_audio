#!/usr/bin/env python3
"""
Debug Level 5c loading - step through and watch PC.
"""
...
def main():
    ...
    state_data = np.array(
        [e_entry & 0xFFFFFFFF, e_entry >> 32, 0, 5, 3, 0, 0, 0, 0, 0, 0, 0, 0, 0, RAM_BASE & 0xFFFFFFFF, RAM_BASE >> 32, 0, 0, 0, 0, 0, 0, 0],
        dtype=np.uint32
    ).tobytes()
    ...
    for i in range(5):
        ...
        print(f"  [{i+1}] PC=0x{state['pc']:016x}, halted={state['halted']}, mode={state['mode']}, trap_pending={state['trap_pending']}")
        if state['halted']:
            break

    uart = core.read_uart_output().decode('latin-1', errors='replace')
    if uart:
        print("UART output:", repr(uart[-100:]))
