#!/usr/bin/env python3
"""
Trap at the panic to see exception details.
"""
...
# Use 20M instructions per dispatch
...
for iteration in range(5):
    ...
    # Get trap info
    trap_cause_lo = cpu_readback['trap_cause'][0][0]
    trap_cause_hi = cpu_readback['trap_cause'][0][1]
    trap_val_lo = cpu_readback['trap_val'][0][0]
    trap_val_hi = cpu_readback['trap_val'][0][1]
    trap_epc_lo = cpu_readback['trap_epc'][0][0]
    trap_epc_hi = cpu_readback['trap_epc'][0][1]
    
    print(f"\nIter {iteration}: {instr_count:8d} instrs, PC=0x{pc:08x}")
    print(f"  Trap cause: 0x{trap_cause_hi:08x}{trap_cause_lo:08x}")
    print(f"  Trap value: 0x{trap_val_hi:08x}{trap_val_lo:08x}")
    print(f"  Trap EPC:   0x{trap_epc_hi:08x}{trap_epc_lo:08x}")
    print(f"  total_irq={total_irq_count}, timer_irq={timer_irq_count}")
