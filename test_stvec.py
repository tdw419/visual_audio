#!/usr/bin/env python3
"""
Check stvec and trapvec setup at panic.
"""
...
for iteration in range(5):
    ...
    priv_mode = cpu_readback['priv_mode'][0]
    
    # M-mode trap vectors
    mtvec_lo = cpu_readback['mtvec'][0][0]
    mtvec_hi = cpu_readback['mtvec'][0][1]
    
    # S-mode trap vectors
    stvec_lo = cpu_readback['stvec'][0][0]
    stvec_hi = cpu_readback['stvec'][0][1]
    ...
    print(f"\nIter {iteration}: PC=0x{pc:08x}, priv_mode={priv_mode}")
    print(f"  mtvec=0x{mtvec_hi:08x}{mtvec_lo:08x}")
    print(f"  stvec=0x{stvec_hi:08x}{stvec_lo:08x}")
    print(f"  scause=0x{scause_hi:08x}{scause_lo:08x} (interrupt={scause_hi>>31 & 1}, code={scause_lo})")
    print(f"  sepc=0x{sepc_hi:08x}{sepc_lo:08x}")
