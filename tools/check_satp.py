#!/usr/bin/env python3
"""
Check SATP state when executing SW.
"""
...
def check_satp():
    """Check SATP value at SW."""
    print(f"\n{'='*70}")
    print("Checking SATP at SW instruction")
    print(f"{'='*70}")
    ...
    for step in range(200000):
        ...
        if pc == 0x80000abc:
            satp_lo, satp_hi = cpu_readback['satp'][0][0], cpu_readback['satp'][0][1]
            satp = (satp_hi << 32) | satp_lo
            
            print(f"\nSATP state at 0x80000abc:")
            print(f"  satp.x = 0x{satp_lo:08x}")
            print(f"  satp.y = 0x{satp_hi:08x}")
            print(f"  satp (full) = 0x{satp:016x}")
            ...
            print(f"  mode = {mode} (8 = SV39)")
            print(f"  asid = {asid}")
            print(f"  ppn = {ppn}")
            
            # Check if MMU is enabled
            if mode == 8:
                print(f"\n  MMU is ENABLED (SV39 mode)")
                print(f"  This might be causing issues with stores!")
            else:
                print(f"\n  MMU is DISABLED")
            return
