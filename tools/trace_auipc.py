#!/usr/bin/env python3
"""
Trace auipc a5, 0xc at 0x80000ab8.
"""
...
def trace_auipc():
    """Trace auipc instruction."""
    print(f"\n{'='*70}")
    print("Tracing auipc a5, 0xc at 0x80000ab8")
    print(f"{'='*70}")
    ...
    for step in range(200000):
        ...
        if pc == 0x80000ab8:
            ...
            pc_lo, pc_hi = cpu_readback['pc'][0][0], cpu_readback['pc'][0][1]
            
            print(f"\nBEFORE auipc a5, 0xc:")
            print(f"  PC = 0x{pc:016x}")
            print(f"  a5 = 0x{a5:016x}")
            
            # Expected: imm = 0xc << 12 = 0xc000 (49152)
            #          sext32_to_64(0xc000) = 0x000000000000c000
            #          a5 = 0x80000ab8 + 0x000000000000c000 = 0x80000cb8
            expected_a5 = pc + 49152
            print(f"  Expected a5 after auipc: 0x{expected_a5:016x}")
            ...
            cpu_readback = np.frombuffer(cpu_readback_bytes, dtype=CPU_DTYPE)
            regs = cpu_readback['regs'][0]
            a5_after = (regs[15][1] << 32) | regs[15][0]
            
            print(f"\nAFTER auipc a5, 0xc:")
            print(f"  a5 = 0x{a5_after:016x}")
            
            if a5_after != expected_a5:
                print(f"\nERROR: a5 is wrong!")
                print(f"  Expected: 0x{expected_a5:016x}")
                print(f"  Got:      0x{a5_after:016x}")
                print(f"  Delta:    0x{a5_after - expected_a5:016x}")
            else:
                print("\nOK: a5 is correct")
            return
