#!/usr/bin/env python3
"""
Single-step trace of the memset loop to find what goes wrong.
"""
...
def trace_memset_loop():
    """Trace the memset loop execution."""
    print(f"\n{'='*70}")
    print("Tracing memset loop execution")
    print(f"{'='*70}")
    ...
    # Create a custom harness with per-instruction CPU readback
    ...
    max_instr_arr = np.array([1], dtype=np.uint32)  # 1 instruction per dispatch for single-stepping
    ...
    # Run until we hit the memset loop, then trace
    print("\nRunning to reach memset...")
    ...
    print(f"Reached PC=0x{pc:016x} after {instr_count} instructions")
    
    # Check if we're in memset
    if 0x80001118 <= pc <= 0x800011a4:
        print("We're in memset - switching to single-step mode")
        
        # Now single-step through the loop
        loop_iters = 0
        prev_pc = None
        stuck_count = 0
        ...
        for step in range(200):
            ...
            a3 = (regs[13][1] << 32) | regs[13][0]  # x13 = a3 (loop counter)
            a6 = (regs[18][1] << 32) | regs[18][0]  # x18 = a6 (loop limit)
            
            print(f"  Step {step:3d}: PC=0x{pc:08x}, a3={a3:4d}, a6={a6:4d}, instr={instr_count}")
            
            # Check if stuck
            if pc == prev_pc:
                stuck_count += 1
                if stuck_count > 5:
                    print(f"\n  STUCK at PC=0x{pc:08x} - PC not advancing!")
                    break
            else:
                stuck_count = 0
                prev_pc = pc
            
            # Check if we exited memset
            if pc < 0x80001118 or pc > 0x800011a4:
                print(f"\n  Exited memset at step {step}")
                break
            
            # Check if the loop should have exited
            if a3 >= a6 and pc == 0x800011a0:
                print(f"\n  Loop should exit: a3={a3} >= a6={a6}, but taking branch back to 0x80001190")
                print(f"  BLTU condition check: a3 < a6 = {a3 < a6}")
                
                # Check 64-bit comparison
                a3_hi = regs[13][1]
                a6_hi = regs[18][1]
                a3_lo = regs[13][0]
                a6_lo = regs[18][0]
                print(f"  a3: lo={a3_lo:08x}, hi={a3_hi:08x}")
                print(f"  a6: lo={a6_lo:08x}, hi={a6_hi:08x}")
                
                if a3_hi < a6_hi:
                    print(f"  lt64: true (a3_hi < a6_hi)")
                elif a3_hi > a6_hi:
                    print(f"  lt64: false (a3_hi > a6_hi)")
                else:
                    print(f"  lt64: {a3_lo < a6_lo} (a3_lo < a6_lo, hi equal)")
                break
            
            if step >= 50:
                print(f"\n  Stopping after {step+1} steps")
                break
    else:
        print("Not in memset - didn't reach it yet")
