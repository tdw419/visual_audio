#!/usr/bin/env python3
"""
Check raw CPU buffer bytes around the srliw instruction.
"""
...
def trace_srliw_raw():
    """Check raw bytes after srliw."""
    print(f"\n{'='*70}")
    print("Tracing srliw raw bytes")
    print(f"{'='*70}")
    ...
    # Find offset of regs[18] in CPU_DTYPE
    regs_dtype = CPU_DTYPE.fields['regs'][0][0]  # Get the (32,2) array dtype
    regs_start_offset = CPU_DTYPE.fields['regs'][1]
    print(f"regs field starts at byte offset {regs_start_offset}")
    print(f"Each element is {regs_dtype.itemsize} bytes")
    print(f"regs[18] starts at byte offset {regs_start_offset + 18 * regs_dtype.itemsize}")
    ...
    for step in range(200000):
        ...
        if pc == 0x80001180:
            ...
            print(f"\nBEFORE srliw:")
            print(f"  a2 = 0x{a2_full:016x} (lo=0x{a2_lo:08x}, hi=0x{a2_hi:08x})")
            print(f"  Raw bytes at regs[18]:")
            a6_offset = regs_start_offset + 18 * 8
            raw_bytes = cpu_readback_bytes[a6_offset:a6_offset+8]
            print(f"    {raw_bytes.hex()}")
            lo = struct.unpack('<I', raw_bytes[0:4])[0]
            hi = struct.unpack('<I', raw_bytes[4:8])[0]
            print(f"    lo=0x{lo:08x}, hi=0x{hi:08x}, full=0x{(hi<<32) | lo:016x}")
            ...
            print(f"\nAFTER srliw:")
            print(f"  a6 = 0x{a6_full:016x} (lo=0x{a6_lo:08x}, hi=0x{a6_hi:08x})")
            print(f"  Expected: 512 (0x0000000000000200)")
            print(f"  Raw bytes at regs[18]:")
            raw_bytes = cpu_readback_bytes[a6_offset:a6_offset+8]
            print(f"    {raw_bytes.hex()}")
            lo = struct.unpack('<I', raw_bytes[0:4])[0]
