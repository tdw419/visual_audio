#!/usr/bin/env python3
"""
Find who calls memset with a6 corrupted.
"""
...
def trace_a6_source():
    """Find where a6 gets corrupted."""
    print(f"\n{'='*70}")
    print("Finding a6 corruption source")
    print(f"{'='*70}")
    ...
    for step in range(200000):
            ...
            if a6_full == 0x81000000:
                ...
                for line in result.stdout.split('\n'):
                    if f"{pc:08x}:" in line:
                        ...
                        # Check if it writes to a6 (x18)
                        # Format: "aa bb cc dd" encoding
                        parts = line.split()
                        if len(parts) >= 3:
                            encoding = parts[1]
                            rd = (int(encoding, 16) >> 7) & 0x1F
                            if rd == 18:
                                print(f"  ^ Instruction writes to x18 (a6)")
                        break
                return
        ...
        if pc == 0x80001158:
            print("\nEntering memset fast path")
            print(f"a6 = 0x{a6_full:016x}")
            print("a6 is ALREADY corrupted!")
