#!/usr/bin/env python3
"""
Test store instruction by reading GPU memory back.
"""
...
def trace_store():
    """Trace store instruction with GPU memory readback."""
    print(f"\n{'='*70}")
    print("Tracing store instruction at 0x80000abc")
    print(f"{'='*70}")
    ...
    for step in range(200000):
        ...
        if pc == 0x80000abc:
            ...
            # Convert bytes to uint8 array, then to pixels
            mem_uint8 = np.frombuffer(mem_readback_bytes, dtype=np.uint8)
            # Reshape to (pixel_count, 4) = (RGBA pixels)
            pixel_idx = offset // 4
            byte_idx = offset % 4
            
            # Read the RGBA pixel
            pixel = mem_uint8[pixel_idx * 4:(pixel_idx + 1) * 4]
            mem_value = pixel[byte_idx]
            print(f"  Memory at 0x{addr:016x} = 0x{mem_value:02x}")
            ...
            # Read memory from GPU again
            mem_readback_bytes = queue.read_buffer(memory_buffer)
            pixel = mem_readback_bytes[pixel_idx * 4:(pixel_idx + 1) * 4]
            mem_value = pixel[byte_idx]
            
            print(f"\nAFTER sw s2, 1784(a5):")
            print(f"  Memory at 0x{addr:016x} = 0x{mem_value:02x}")
            print(f"  Expected: 0x01 (s2 = 0x{s2:016x})")
            
            if mem_value == (s2 & 0xFF):
                print("\nOK: Store worked!")
            else:
                print(f"\nERROR: Store failed!")
            return
