#!/usr/bin/env python3
"""
Find where a6 gets corrupted before srliw.
"""
...
def find_a6_corruption():
    """Trace where a6 gets 0x81000000."""
    print(f"\n{'='*70}")
    print("Finding where a6 gets corrupted")
    print(f"{'='*70}")
    ...
    # Watch for when a6 changes to 0x81000000
    prev_a6 = None
    ...
    for step in range(200000):
        ...
        if 0x80001158 <= pc <= 0x800011a4:
            if prev_a6 is None:
                prev_a6 = a6_full
                print(f"\nEntering memset at 0x80001158:")
                print(f"  a6 = 0x{a6_full:016x}")
            elif a6_full != prev_a6:
                print(f"\na6 changed at 0x{pc:08x} (step {step}):")
                print(f"  Was: 0x{prev_a6:016x}")
                print(f"  Now: 0x{a6_full:016x}")
                
                if a6_full == 0x81000000:
                    print("  FOUND THE CORRUPTION!")
                    
                    # Check if we're executing an instruction that writes to a6
