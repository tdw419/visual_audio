...
def build_memory_image_no_mmu(elf_path: str) -> Tuple[np.ndarray, int]:
    ...
    for seg in elf.program_headers:
        if elf.entry_point >= seg['vaddr'] and elf.entry_point < seg['vaddr'] + seg['memsz']:
            ...
            entry_offset_in_seg = elf.entry_point - seg['vaddr']
            break
    
    # Load all segments at physical addresses, remapping from high virtual
    # Segment at 0x7ffff000 loads at 0, so entry (0x80000000) maps to 0x1000
    base_offset = entry_seg_vaddr  # Base virtual address
    ...
    for seg in elf.program_headers:
        ...
        # Calculate physical address by subtracting base offset
        seg_start = seg['vaddr'] - base_offset
        entry_remapped = elf.entry_point - base_offset
    ...
    entry_remapped = elf.entry_point & 0xFFFFFF
    print(f"  Memory: {memory_size // 1024}KB ({num_pixels} pixels)")
    print(f"  Entry point (remapped): 0x{entry_remapped:016x}")

    return pixels, entry_remapped
...
if __name__ == '__main__':
    ...
    # Save pixel dump
    np.save('test_kernel_no_mmu_v2.npy', pixels)
    print(f"\n[5] Saved pixel dump: test_kernel_no_mmu_v2.npy")

    # Verify entry point instruction
    entry_word = pixels[entry_point // 4]
    word_value = entry_word[0] | (entry_word[1] << 8) | (entry_word[2] << 16) | (entry_word[3] << 24)
    print(f"  Entry instruction: 0x{word_value:08x}")

    print("\nTo boot on GPU without MMU:")
    print(f"  python3 boot_gpu_execute_no_mmu.py test_kernel_no_mmu_v2.npy 0x{entry_point:x}")