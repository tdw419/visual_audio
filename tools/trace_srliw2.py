...
Single-step from start of memset.
"""
...
def trace_srliw():
    ...
    print(f"\n{'='*70}")
    print("Tracing memset execution from start")
    print(f"{'='*70}")
    ...
    # Single step until we hit memset (PC=0x80001158 is entry to fast path)
    print("\nSingle-stepping to find memset...")
    ...
    for step in range(200000):
        ...
        if 0x80001158 <= pc <= 0x800011a4:
            ...
            if pc == 0x80001180:
                # At srliw - dump state before executing
                a2_lo = regs[12][0]
                a2_hi = regs[12][1]
                a2_full = (a2_hi << 32) | a2_lo
                print(f"\nBEFORE SRLIW at 0x80001180:")
                ...
                if a6_full != 512:
                    print(f"  BUG: Got {a6_full}, expected 512!")
                    print(f"  srliw should shift low 32 bits right by 3, then sign-extend")
                    print(f"  Calculation: 0x{a2_lo:08x} >> 3 = 0x{(a2_lo >> 3):08x} = {a2_lo >> 3}")
                    return
                else:
                    print(f"  OK!")
                    return
            ...
            if step % 1000 == 0:
                print(f"Step {step}: in memset at 0x{pc:08x}")
