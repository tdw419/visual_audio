#!/usr/bin/env python3
"""
Fixed RISC-V Hello World test

The issue: WGSL buffer expects u32 channels, not uint8.
We need to pack each byte into its own u32 field.
"""
...
def main():
    ...
    print('Instructions:')
    print(f'  [0] 0x{kernel_words[0]:08x} = lui a0, 0x00100')
    print(f'  [1] 0x{kernel_words[1]:08x} = addi a0, a0, 0x010')
    print(f'  Expected final a0 = 0x00101000')

    # Pack into u32 array for WGSL
    # WGSL struct has u32 fields, so we need 4 u32 per pixel
    pixel_data = np.zeros((4096 * 4096 * 4,), dtype=np.uint32)

    for i, word in enumerate(kernel_words):
        # WGSL expects: pixel.r = byte0, pixel.g = byte1, pixel.b = byte2, pixel.a = byte3
        # But as u32 values, not uint8
        base_idx = i * 4
    ...
    # Create ROM buffer (now using u32, not uint8)
