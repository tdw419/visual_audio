#!/usr/bin/env python3
"""
Smart Roadmap Task Executor - executes pending tasks with executable commands
"""
import re
import sys
import os
...
def extract_executable_command(test_text):
    """Extract an executable command from test text"""
...
def parse_roadmap():
    ...
    # Only keep tasks with executable commands
    executable_tasks = []
    for t in filtered_tasks:
        if t['test_command']:
            cmd = extract_executable_command(t['test_command'])
            if cmd and 'manual' not in t['test_command'].lower():
                t['executable_cmd'] = cmd
...
def main():
    ...
    if not pending:
        print("No pending tasks with executable tests found")
        return 0
    
    priority_order = {'CRITICAL': 0, 'HIGH': 1, 'MEDIUM': 2, 'LOW': 3}
    pending.sort(key=lambda t: priority_order.get(t['priority'], 99))
    
    print(f"Found {len(pending)} pending tasks with executable tests:")
    for i, t in enumerate(pending, 1):
        print(f"{i}. {t['id']}: {t['title']}")
        print(f"   Priority: {t['priority']}")
        print(f"   Phase: {t['phase']}")
        print(f"   Command: {t['executable_cmd']}")
        print()
    ...
    if task['executable_cmd']:
        print(f"\nRunning command: {task['executable_cmd']}")
        try:
            ...
            if result.returncode == 0:
                ...
                for i, line in enumerate(lines):
                    if f"- [ ] **{task['id']}**" in line:
                        ...
                        lines.insert(i + 2, f"  - Receipt: Executed by smart roadmap executor at {Path(__file__).stat().st_mtime}")
                        break
